/**
 * MD5 hash utility for Pixiv client hash calculation
 */
...
export function md5(input: string): string {
  // Use pure JS implementation since Web Crypto doesn't support MD5
...
function md5Pure(input: string): string {
  ...
  const hex = (x: number[]) => x.map((n) => rhex(n)).join("");

  return hex(md51(input));
}

/**
 * Calculate Pixiv client hash
 * Format: MD5(time + hash_secret)
 * @param time ISO timestamp
 * @returns Client hash string
 */
export function calcClientHash(time: string): string {
  const hashSecret = "28c1fdd170a5204386cb1313c7077b34f83e4aaf4aa829ce78c231e05b0bae2c";
  return md5(time + hashSecret);
}
